...
class PretrainDataset_NegSampling(PretrainDataset):
    N = 50
    PDF = norm()
    ...
    def __init__(self, args, user_seq, long_sequence):
        ...
        cnt_series = user_item_time["item"].value_counts()[:self.N]
        
        self.weight = [self.PDF.pdf(i * 4 / self.N) for i in range(self.N)]
        self.topN = cnt_series.index.to_list()
        
        super().__init__(args, user_seq, long_sequence)
        
    def neg_sample(self, item_set):
        item = random.choices(self.topN, weights=self.weight)[0]
        while item in item_set:
            item = random.choices(self.topN, weights=self.weight)[0]
        return item
    
    def extract_samples(self, sequence, length):
        not_watched = [i for i in self.topN if not i in sequence]
        l = len(not_watched)
        assert not_watched, f"""해당 사용자가 Top {self.N} 안의 모든 영화를 소비함. 
        {self.__class__.__name__}의 필드 N 값을 증가시키기를 권장함."""
        return random.choices(not_watched, weights=self.weight[:l], k=length)
    ...
    def __getitem__(self, index):
        ...
        for item in sequence[:-1]:
            ...
            if prob < self.args.mask_p:
                masked_item_sequence.append(self.args.mask_id)
                neg_items.append(self.neg_sample(item_set))
            else:
                masked_item_sequence.append(item)
                neg_items.append(item)
        ...
        if len(sequence) < 2:
            ...
            # neg_start_id = random.randint(0, len(self.long_sequence) - sample_length)
            pos_segment = sequence[start_id : start_id + sample_length]
            neg_segment = self.extract_samples(sequence, sample_length)
            masked_segment_sequence = (
                sequence[:start_id]
                + [self.args.mask_id] * sample_length
                + sequence[start_id + sample_length :]
...
class SASRecDataset_NegSampling(SASRecDataset):
    N = 50
    PDF = norm()
    ...
    def __init__(self, args, user_seq, test_neg_items=None, data_type="train"):
        ...
        super().__init__(args, user_seq, test_neg_items, data_type)
    ...
    def __getitem__(self, index):
        ...
        seq_set = set(items)
        for _ in input_ids:
            target_neg.append(self.neg_sample(seq_set))
...
if __name__ == '__main__':
    from utils import get_user_seqs_long, get_item2attribute_json
    class dummy():
        pass
    args = dummy()
    args.max_seq_length = 50
    args.mask_p = 0.2
    args.data_dir = '/opt/ml/input/data/train/'
    args.data_name = "Ml"
    
    user_seq, max_item, long_sequence = get_user_seqs_long(args.data_dir + "train_ratings.csv")
    item2attribute, attribute_size = get_item2attribute_json(args.data_dir + args.data_name + "_item2attributes.json")
    ...
    k = PretrainDataset_NegSampling(args, user_seq, long_sequence)
    m = PretrainDataset(args, user_seq, long_sequence)
...
if __name__ == '__main__':
    # 실제 뽑힌 값들의 분포
    df = pd.DataFrame(
        k.extract_samples([356, 4993], 100000)
        )
    plt.plot(df.value_counts().tolist())
    
# %%
if __name__ == '__main__':
    compare = zip(k.topN, df.value_counts().index.tolist()).__iter__()
    print('topN\tpred')
    print('='*15)
    for _ in range(20):
        topN, pred = compare.__next__()
        print(f'{topN}\t{pred[0]}')
